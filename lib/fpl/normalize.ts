...
export function normalizeTeamPicks(rawPicksPayload: unknown): number[] {
  if (typeof rawPicksPayload !== "object" || rawPicksPayload === null) {
    return [];
  }

  const payload = rawPicksPayload as { picks?: unknown[] };
  const picks = Array.isArray(payload.picks) ? payload.picks : [];

  return picks
    .filter((entry) => typeof entry === "object" && entry !== null)
    .map((entry) => toNumber((entry as Record<string, unknown>).element, 0))
    .filter((id) => id > 0);
}

export function resolveCurrentGameweek(bootstrapRaw: unknown): number | null {
  if (typeof bootstrapRaw !== "object" || bootstrapRaw === null) {
    return null;
  }
  ...
  for (const event of events) {
    if (typeof event === "object" && event !== null && "is_current" in event) {
      const maybeCurrent = event as { is_current?: unknown; id?: unknown };
      if (maybeCurrent.is_current === true) {
        const eventId = toNumber(maybeCurrent.id, 0);
        if (eventId > 0) {
          return eventId;
        }
      }
