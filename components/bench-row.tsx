...
export function BenchRow({ bench, teamShortNames, selectedPlayerId, onSelect }: BenchRowProps) {
  return (
    <section className="premium-panel rounded-card border border-border/70 p-3 md:p-3.5">
      ...
      <div className="grid gap-1.5 sm:grid-cols-4">
        {bench.map((player, index) => (
          <button
            ...
            className={`rounded-xl px-2.5 py-1.5 text-left transition focus-visible:outline-none ${
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <p className="text-xs font-semibold text-brand">{player.projectedPoints.toFixed(1)} pts</p>
            </div>
            <p className="mt-0.5 truncate text-sm font-medium leading-tight text-white">{player.webName}</p>
            <p className="text-[10px] uppercase tracking-[0.08em] text-muted">
              {teamShortNames[player.teamId] ?? `T${player.teamId}`} · {player.position}
            </p>
            <p className="mt-0.5 text-[10px] text-muted">5+ {player.chanceOfFivePlusPoints.toFixed(1)}%</p>
          </button>
        ))}
      </div>
    </section>
